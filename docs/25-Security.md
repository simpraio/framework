...
The `http-client` extension enforces an egress allowlist (`extensions.http-client.egress`): when enabled, requests may only target allowlisted hosts, and with `block_private_ips` on, an allowlisted host that resolves to a **non-globally-routable** IP is rejected as defense in depth. That is wider than private and reserved: shared address space (`100.64.0.0/10`, carrier-grade NAT) and the benchmarking and documentation ranges are refused too. Special-purpose ranges RFC 6890 marks globally reachable, such as the NAT64 prefix `64:ff9b::/96`, stay allowed - being special-purpose is not the test, being unroutable is. Reachability comes from a static classification of the IANA registries, matched longest-prefix-first - IPv6 denied by default, with `2000::/3` and the explicitly globally-reachable special-purpose carve-outs allowed; IPv4 allowed outside its special-purpose carve-outs - rather than from PHP's `FILTER_FLAG_GLOBAL_RANGE` - which accepts several non-global ranges (multicast, `64:ff9b:1::/48`, `3fff::/20`, `5f00::/16`). The flag applies only where the table says nothing, and can then withhold an address but never permit one. IPv4-mapped addresses (`::ffff:0:0/96`) are refused as a range. A host resolving to nothing at all is rejected too, since there is then no address to check. The validated addresses are pinned onto the connection (`CURLOPT_RESOLVE`) so the name is not resolved a second time and dialled at an address that was never checked. Redirect targets are checked against the same policy before they are followed. The outbound proxy is operator-config only and is never accepted as a per-request option (a request-supplied proxy would bypass the allowlist). It is also always sent to cURL, empty when unset, so that a `http_proxy`/`HTTPS_PROXY`/`ALL_PROXY` variable in the environment cannot route outbound traffic unannounced; when a proxy is configured, `CURLOPT_NOPROXY` is sent empty so `NO_PROXY` cannot exempt a host from it and connect directly. Address pinning only covers direct connections - a proxy resolves the destination itself - so a configured proxy is refused at boot while the egress guard is enabled with `block_private_ips`. A disabled guard enforces no addresses, so a proxy is accepted there. See [Extension: HTTP Client](16-Extensions-Httpclient.md).
